    ...
    dataBaseConnection
      ...
      .catch(error => {
        ...
        if (error.code === "ER_NO_SUCH_TABLE") {
          console.log(
            `No se encontró la tabla con nombre ${tabla.nombre}, error sql: ${error.sqlMessage}`
          );
          /* creamos la tabla, es equivalente a:
              CREATE TABLE ${nombreTabla};
              */
          dataBaseConnection.schema
            .createTable(`${tabla.nombre}`, nuevaTabla => {
              ...
              if (tabla.atributos && tabla.atributos.length > 0) {
                ...
                tabla.atributos.map(atribu => {
                  ...
                  // si el atributo es clave foranea, lo creamos como tal con la referencia a la tabla foranea
                    ...
                    nuevaTabla
                      .foreign(`${atribu.nombre}`)
                      .references(`${atribu.referenceAtribbute}`)
                      .inTable(`${atribu.referenciaTable}`)
                      .onUpdate("CASCADE");
                  }
              ...
              // si no hay atributos, lanzamos error
                ...
                throw new Error(
                  "No se encontró atributos en la tabla, por favor ajuste el archivo tablas.json"
                );
              }
            })
            .then(respuesta =>
              console.log(`Tabla ${tabla.nombre} creada con éxito`)
            )
            .catch(mistake => {
              console.log(mistake);
            });
