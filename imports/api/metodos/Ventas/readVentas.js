      ...
      dataBaseConnection
        .select(
          dataBaseConnection.raw(
            "codigo, fecha, EMPLEADO.nombre_completo as empleado, CLIENTE.nombre_completo as cliente, TIMESTAMPDIFF(DAY,fecha,CURDATE()) AS fechaTranscurrida"
          )
        )
        .from(dataBaseConnection.raw("VENTA, CLIENTE, EMPLEADO"))
        .whereRaw(
          "EMPLEADO.cedula = VENTA.empleadoCedula and CLIENTE.cedula = VENTA.clienteCedula"
        )
        .then(respuesta => {
