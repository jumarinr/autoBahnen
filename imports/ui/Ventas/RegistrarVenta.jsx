...
export default class RegistrarVenta extends React.Component {
    ...
    llenarDatos(event){
      ...
      if(!this.state.fecha){
        this.setState({openError: true, msgError: "Debe ingresar una fecha de venta"})
        return false;
      }
        ...
        <hr/>
          ...
          <CardContent>
            ...
            <form onSubmit={event =>  this.llenarDatos(event) }>
                ...
                <Grid item xs={12}>
                <TextField id="fecha" fullWidth={true} type="date" required //label="Ingrese fecha" 
                onChange={(event=>{this.changeValue(event, 'fecha')})} value={this.state.fecha}/>
                </Grid>
                <br/>
                <Grid item xs={12}>
